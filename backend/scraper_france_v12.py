"""
Padel Alert â€” Scraper France v12 â€” submit_main pour toutes les pages

Fix v12 : submit_main (non submit_page) pour toutes les pages de pagination.
Diagnostic v11 : submit_main (page=0) applique le filtre comitÃ© via session PHP,
mais submit_page (pages 1+) ignorait ce filtre â†’ retournait toute la ligue.
En forÃ§ant submit_main pour chaque page, le filtre comitÃ© est rÃ©-appliquÃ© Ã 
chaque requÃªte â†’ couverture 100% attendue si Drupal respecte page=N avec submit_main.

Cumul des fixes :
- [all]=0 dans ligue_sumbit/ajax (v11) â†’ filtre comitÃ© correct
- submit_main pour toutes les pages (v12) â†’ filtre persistant sur pagination
...
def set_comite_filter(session, ligue_id, comite_id, comite_name):
    ...
    [all]=0 = "seuls les comitÃ©s listÃ©s" â†’ filtre effectif (confirmÃ© capture rÃ©seau 09/05/2026).
    [all]=1 = "tout sÃ©lectionner" â†’ retourne toute la ligue (bug v8-v10).
    """
    data = {
...
def ajax_page(session, fbid, page_num, date_start, date_end):
    """
    RequÃªte AJAX /system/ajax â€” toujours submit_main pour rÃ©-appliquer le filtre comitÃ©.
    submit_page ignorerait le filtre de session â†’ on utilise submit_main + page=N.
